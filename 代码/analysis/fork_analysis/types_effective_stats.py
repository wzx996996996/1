...
# ---------------------- 步骤4：识别Type 2和Type 3（仅统计实际存在的fork） ----------------------
def get_type2_type3_forks(type1_forks, all_forks, original_baseline):
    ...
    for fork_id, father_repo_id in tqdm(unique_forks, desc="检测Type 2/3"):
        ...
        # 既不是Type 2也不是Type 3，后续计入纯贡献型
        pass

    return type2, type3, skipped_forks
...
def main():
    ...
    # 步骤4：检测Type 2/3（仅统计实际存在的fork）
    type2, type3, skipped_forks = get_type2_type3_forks(type1_all, all_forks, original_baseline)

    # 计算实际有效fork数量（排除被跳过的）
    actually_processed = len(type1_all) - len(skipped_forks)
    pure_contribution = actually_processed - (len(type2) + len(type3))  # 实际纯贡献型
    ...
    print("--------------------------------------")
    print(f"实际纯贡献型forks（仅Type 1）：{pure_contribution}个")
